...
class module.exports.Twinkie
  ...
  coffee: (from, to) ->
    find = (commands, from, to) ->
      ...
      try
        for file in fs.readdirSync from
          if match = /^(.*?)_?.coffee$/.exec(file)
            stat = null
            [ base, dash ] = match.slice(1)
            source = "#{from}/#{file}"
            try
              try
                stat = fs.statSync("#{to}/#{base}.js")
               catch e
                throw e if e.code isnt "ENOENT"
                try
                  stat = fs.statSync("#{to}/#{base}")
                catch e
                  throw e if e.code isnt "ENOENT"
      ...
      if files.length
        ...
        for file in files
          if match = /([^/]*)_\.coffee$/.exec file
            commands.push [ "node-streamline", [ "-c", "#{to}/#{match[1]}_.js" ] ]
            commands.push [ "rm", [ "#{to}/#{match[1]}_.js" ] ]
    ...
    find = (commands, from, to) ->
      ...
      if files.length
        ...
        for file in files
          ...
          commands.push [ "lessc", [ file, "#{to}/#{base}.css" ] ]
